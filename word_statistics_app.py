import os
import click
from WSM import WordStatsManager
    ...
    for path in paths:
        ...
        if format not in VALID_FORMATS:
            raise click.BadParameter(
                f"Output file {path} has an invalid format {format}. Known formats are {', '.join(VALID_FORMATS)}.")
...
def main(number, output_spec, input_paths):
    wsm = WordStatsManager(number, input_paths, output_spec)
    wsm.process()
